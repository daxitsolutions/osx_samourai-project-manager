        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                Grid(horizontalSpacing: 16, verticalSpacing: 16) {
                    GridRow {
                        detailCard(title: "Projets affectés", value: assignedProjectNames.isEmpty ? "Aucun" : assignedProjectNames.joined(separator: ", "))
                ...
                VStack(alignment: .leading, spacing: 12) {
                    Text("Évaluation continue")
                        .font(.title2.weight(.semibold))

                    HStack(spacing: 16) {
                        let alertsValue = performanceSnapshot?.alerts.isEmpty == false
                            ? performanceSnapshot?.alerts.map(\.kind.label).joined(separator: ", ") ?? "Aucune"
                            : "Aucune"
                        detailCard(title: "Dernière note", value: performanceSnapshot?.latestScore.map { String(format: "%.2f/5", $0) } ?? "Pas de note")
                        detailCard(title: "Tendance", value: performanceSnapshot?.trend.label ?? "Stable")
                        detailCard(title: "Alertes", value: alertsValue)
                    }

                    if scopedEvaluations.isEmpty {
                        Text("Aucune évaluation enregistrée pour cette ressource.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    } else {
                        VStack(alignment: .leading, spacing: 8) {
                            ForEach(Array(scopedEvaluations.suffix(5).reversed())) { evaluation in
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("\(evaluation.evaluatedAt.formatted(date: .abbreviated, time: .omitted)) • \(evaluation.milestone)")
                                        .font(.caption.weight(.semibold))
                                    Text("Score pondéré: \(String(format: "%.2f/5", evaluation.weightedScore)) • Évaluateur: \(evaluation.evaluator)")
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
